<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$pdo->errorInfo();
			$sql = "INSERT INTO events (event_date, event_time, event_location, event_description) VALUES(?, ?, ?, ?)";	//change to evet_ghiooghdf
			echo "got here5";
			$q = $pdo->prepare($sql)->execute(array($date,$time,$location,$description));
			echo "got here6";
			Database::disconnect();
			echo "got here7";
